...
function App() {
  const [explorerWidth, setExplorerWidth] = useState(400); // initial width in px
  ...
  const resize = useCallback((e: MouseEvent) => {
    if (isResizing && containerRef.current) {
      ...
      // Calculate potential new width relative to the right edge
      const newWidth = containerWidth - (e.clientX - containerRef.current.offsetLeft);

      // LIMITS: 
      // Min: 320px (Explorer shouldn't disappear)
      // Max: containerWidth - 400px (Viewport should always have at least 400px space)
  ...
  return (
    <div className="h-screen w-screen bg-black text-white flex flex-col p-6 md:p-10 font-sans relative overflow-hidden select-none" ref={containerRef}>
      {/* Background Decor */}
      ...
      <nav className="flex items-center justify-between mb-8 md:mb-16 relative z-20 shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="w-9 h-9 md:w-11 md:h-11 flex items-center justify-center hover:bg-white/10 rounded-xl md:rounded-2xl transition-all border border-white/5">
      ...
      <main className="flex-1 flex flex-col md:flex-row gap-0 relative z-10 min-h-0">
          ...
          >
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center p-4">
              ...
              >
                ...
                <div className="w-16 h-16 md:w-24 md:h-24 rounded-2xl md:rounded-[36px] bg-white text-black flex items-center justify-center shadow-[0_0_40px_rgba(255,255,255,0.2)] group-hover/btn:shadow-[0_0_100px_rgba(255,255,255,0.5)] group-hover/btn:rotate-[360deg] transition-all duration-1000 ease-[0.16, 1, 0.3, 1] relative z-10">
                  <Upload size={24} strokeWidth={2.5} className="md:hidden" />
                  <Upload size={32} strokeWidth={2.5} className="hidden md:block group-hover/btn:scale-125 transition-transform duration-500" />
                </div>
                <div className="text-center space-y-2 md:space-y-3 relative z-10">
                  <h2 className="text-lg md:text-xl font-black tracking-[-0.02em] uppercase group-hover/btn:tracking-[0.1em] transition-all duration-700">Initialize Project</h2>
                  <p className="text-[8px] md:text-[10px] font-mono text-white/20 tracking-[0.3em] uppercase">Drop RAW Footage</p>
                </div>
              </motion.button>
            </div>
            ...
            <div className="absolute bottom-8 left-8 right-8 flex items-center justify-between">
              <div className="hidden sm:flex gap-4">
                <div className="flex flex-col">
                  <span className="text-[8px] md:text-[9px] font-black text-white/10 uppercase tracking-widest">Resolution</span>
        ...
        {/* Resize Handle (Splitter) - Hidden on Mobile */}
        <div
        ...
        >
          {/* Visual Divider Line */}
          <div className={`w-[1px] h-[60%] transition-all duration-300 ${isResizing ? 'bg-white h-full shadow-[0_0_20px_rgba(255,255,255,0.8)]' : 'bg-white/10 group-hover/handle:bg-white/40 group-hover/handle:h-[80%]'}`} />

          <div className={`absolute w-1.5 h-12 bg-white rounded-full transition-all duration-500 opacity-0 group-hover/handle:opacity-100 ${isResizing ? 'scale-y-150 h-24' : 'scale-y-100'}`} />
        ...
        {/* Right: Project Explorer with Thumbnails */}
        <div
          className="flex flex-col gap-6 md:gap-8 min-h-0 shrink-0 md:pl-8 w-full md:w-auto overflow-hidden"
          style={{ width: typeof window !== 'undefined' && window.innerWidth < 768 ? '100%' : explorerWidth }}
        >
          <div className="flex items-center justify-between px-2">
            <h3 className="text-[10px] md:text-[11px] font-black tracking-[0.4em] uppercase text-white/30 truncate">Assets</h3>
            ...
            {projects.map((project) => (
              ...
              >
                ...
                <div className="relative w-20 h-14 md:w-28 md:h-20 rounded-xl md:rounded-2xl overflow-hidden bg-white/5 border border-white/5 shrink-0">
                  <img src={project.thumb} alt="" className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-300 opacity-60 group-hover:opacity-100" />
                </div>

                <div className="flex flex-col justify-center flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-[11px] md:text-sm font-bold text-white/80 group-hover:text-white truncate tracking-tight">{project.name}</span>
                  </div>
                  <div className="flex flex-col gap-0.5 md:gap-1">
                    <span className="text-[8px] md:text-[10px] font-mono text-white/20 tracking-tighter truncate">{project.meta}</span>
                    <div className="text-[8px] md:text-[9px] text-white/40 font-bold uppercase tracking-widest">{project.date}</div>
                  </div>
                </div>
              </motion.div>
            ))}
